...
#define DEBUG 0
#define LOG_TAG "NativeFilter"
#include "jni_log.h"


#define JNIREG_CLASS "com/android/picshow/edit/editor/filters/NativeFilter"
...
jintArray gray(JNIEnv *env, jobject obj, jintArray srcPixels, jint width, jint height,
    ...
    LOGI("gray : %d, %d, %d", width, height, env->GetArrayLength(srcPixels));
    jint * pixels = 0;
...
jintArray comics(JNIEnv *env, jobject obj, jintArray srcPixels, jint width, jint height,
    ...
    jint* pixels = env->GetIntArrayElements(srcPixels, JNI_FALSE);
    if (pixels == NULL) {
        return srcPixels;
    }
...
jintArray neon(JNIEnv *env, jobject obj, jintArray srcPixels, jint width, jint height) {
    ...
    int curr_a, curr_r, curr_g, curr_b, right_r, right_g, right_b, bottom_r, bottom_g, bottom_b;
    ...
    for (int i = 0; i < height; i++) {
        ...
        for (int j = 0; j < width; j++) {
            ...
            int bottonIndex = (i + i) * width + j;

            if(currentIndex > newSize || rightIndex > newSize || bottonIndex > newSize)
                continue;

            int currentPixel = cbuf[currentIndex];

            int rightPixel = cbuf[rightIndex];

            int bottomPixel = cbuf[bottonIndex];

            curr_a = alpha(currentPixel);
            curr_r = red(currentPixel);
            curr_g = green(currentPixel);
            curr_b = blue(currentPixel);

            right_r = red(rightPixel);
            right_g = green(rightPixel);
            right_b = blue(rightPixel);

            bottom_r = red(bottomPixel);
            bottom_g = green(bottomPixel);
            bottom_b = blue(bottomPixel);

            int new_r = (int) (2 * sqrt(((curr_r - right_r) * (curr_r - right_r)
                                         + (curr_r - bottom_r) * (curr_r - bottom_r))));
            int new_g = (int) (2 * sqrt(((curr_g - right_g) * (curr_g - right_g)
                                         + (curr_g - bottom_g) * (curr_g - bottom_g))));
            int new_b = (int) (2 * sqrt(((curr_b - right_b) * (curr_b - right_b)
                                         + (curr_b - bottom_b) * (curr_b - bottom_b))));

            rbuf[currentIndex] = ARGB(curr_a, new_r, new_g, new_b);
